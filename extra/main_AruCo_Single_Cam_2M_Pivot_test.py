...
if __name__ == '__main__':
    ...
    while True:
        ...
        if(all(is_marker_detected) ):
            ...
            d_m1 = calculate_distance(np.mean(corner_points[0],axis=0) , np.mean(corner_points[1],axis=0))
            d_m2 = calculate_distance(np.mean(corner_points[2],axis=0) , np.mean(corner_points[3],axis=0))
            d_m1m2_01 = calculate_distance(np.mean(corner_points[0],axis=0) , np.mean(corner_points[2],axis=0))
            d_m1m2_02 = calculate_distance(np.mean(corner_points[1],axis=0) , np.mean(corner_points[3],axis=0))
            print("Distance d_m1: " , d_m1) 
            print("Distance d_m2: " , d_m2) 
            print("Distance d_m1m2_01: " , d_m1m2_01) 
            print("Distance d_m1m2_02: " , d_m1m2_02) 
            
            v1 = np.mean(corner_points[0],axis=0) - np.mean(corner_points[1],axis=0)
            v2 = np.mean(corner_points[2],axis=0) - np.mean(corner_points[3],axis=0)
            angle = anlge_between_two_vector(v1,v2)
            
            p1 = ((np.mean(corner_points[0],axis=0) + np.mean(corner_points[1],axis=0))/2) +  0.1*v1/np.linalg.norm(v1)
            p2 = ((np.mean(corner_points[2],axis=0) + np.mean(corner_points[3],axis=0))/2) +  0.1*v2/np.linalg.norm(v2)
            
            print("Angle between two vecotr:", angle)
            print("dst between two pivot points:", np.linalg.norm(p1-p2))
            print("angle between two aruco 1:", angle_between_two_aruco(corner_points[0],corner_points[1]))
            print("angle between two aruco 2:", angle_between_two_aruco(corner_points[2],corner_points[3]                                                                                                                                                                                              ))                                                                                         
